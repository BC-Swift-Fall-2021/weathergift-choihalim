        ...
        let task = session.dataTask(with: url) { (data, response, error) in
            ...
            do {
                ...
                for index in 0..<result.daily.count {
                    ...
                    let dailyHigh = result.daily[index].temp.max.rounded()
                    let dailyLow = result.daily[index].temp.min.rounded()
                    let dailyWeather = DailyWeather(dailyIcon: dailyIcon, dailyWeekday: dailyWeekday, dailySummary: dailySummary, dailyHigh: Int(dailyHigh), dailyLow: Int(dailyLow))
                ...
                if lastHour > 0 {
                    for index in 1...lastHour {
                        ...
                        let hourlyTemperature = result.hourly[index].temp.rounded()
                        let hourlyWeather = HourlyWeather(hour: hour, hourlyTemperature: Int(hourlyTemperature), hourlyIcon: hourlyIcon)
